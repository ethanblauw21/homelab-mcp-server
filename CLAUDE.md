...
| `docker_read_file` / `docker_write_file` | Docker container file I/O (host-visible fast path / `docker cp` relay; `dryRun`) | 008 / 016 |
| `docker_inspect` | Structured, secret-redacted single-container view (image/`imageId` pin, mounts, ports, restart policy, compose labels, name-kept value-redacted env; `fields?` narrows) | 016 |
| `docker_stats` | Point-in-time `--no-stream` resource snapshot, mem-sorted | 016 |
...
| `verify_integrity` | Read-only drift report: diff forest vs baseline, classify each leaf explained/unexplained; `smart` escalation; `mode` seeded-vs-compared signal; optional audited auto-accept | 009 / 018 |
...
| `service_status` | Parsed systemd unit state `{active, sub, enabled, since, mainPid}` via `systemctl show` (host or LXC; read-only, not audited) | 020 |
| `service_logs` | Bounded, **always-redacted** systemd journal tail — `tail_log` with a `unit`-only contract (reuses `buildTailCommand`) | 020 |
| `service_restart` | `systemctl restart <unit>` (host or LXC) — confirm-gated mutation, structured audit row | 020 |
| `tcp_ping` | One TCP connect from the Windows host → `{reachable, latencyMs}` (zero credentials; read-only, not audited) | 020 |
| `http_probe` | HTTP(S) reachability/status probe → `{status, ok, latencyMs, bodyBytes, from}`; `expectStatus` asserts, `fromVmid` probes inside an LXC (read-only, not audited) | 020 |
| `search_file_regex` | Content-addressed regex read: each match + N context lines (`grep -C` balloon), capped with overflow marker; host/LXC/Docker (read-only, not audited) | 020 |
...
> **Raw-exec gap closers (ADR-020) — "dedicated tool beats raw exec," applied to the last three enumerable patterns.** Three structured shells over existing plumbing, each replacing recurring free-form `execute`/`pct_exec` text. **systemd front door** (`service_status`/`service_logs`/`service_restart`, `tools/serviceTools.ts` + pure `serviceHelpers.ts`): `systemctl show` key=value parse, `service_logs` *delegates to `tailLogHandler`* (full reuse — `since` grammar, line cap, mandatory redaction), `service_restart` is the only mutation (confirm-gated, audited as `service_restart`/`hashScope:"unknown"`). The whole trio's tier **follows the target kind** via `assertTargetTier` (host unit ⇒ root, LXC ⇒ companion) — deliberately stricter than `tail_log` even for the read-only pair; `tail_log` stays the companion host-journal escape hatch. **Probes** (`tcp_ping`/`http_probe`, `tools/probes.ts`): host-side via Node `net`/`http` (observe floor, zero credentials, no node round-trip), `http_probe.fromVmid` shells `pct exec curl` (companion, asserted at runtime); `from: "host" | "vmid:<n>"` names the namespace; **TLS is not verified** (reachability/status check, not a trust check). **`search_file_regex`** (`tools/searchFileRegex.ts`): content-addressed `grep -C` "balloon" — find first, return only the matched line + N context lines each side (`-m max+1` detects overflow; pure parser reconstructs before/after from line numbers, robust to merged groups). Honest deviation: `grep` runs **remotely** (host shell / `pct exec` / `docker exec`), reusing the read family's surface-selection + `validatePath` but not its byte transfer — only matched neighborhoods transit. Tier follows target kind (host ⇒ root, LXC/Docker ⇒ companion). All read-only except `service_restart`; none add a new transport/store/dependency.

> **Output budgeting (ADR-017) — the output side of ADR-011 §1's token-economy doctrine.** Three existing read tools gain opt-in/scoping levers + one new scoped tool, all pure reshaping over data already computed. **`query_audit`** takes `cmdMaxChars` (opt-in: truncate each record's `cmd` to a head window + `…(+N chars)` marker) / `cmdFull` (force verbatim) — the on-disk log is untouched, and truncation is **off by default** (it can hide a forensically-relevant flag). **`health_check`** drops pseudo/virtual mounts (`/dev`, `/run/*`, `/sys/*`, `/proc/*` — `isPseudoMount`) from the storage findings **by default**; `includePseudoFs: true` restores them, and the **rollup status is computed pre-filter** so a hidden mount never changes the verdict. **`describe_homelab`** adds `depth: "status"` between `summary` and `full` — identity + run-state + `snapshotCapable`, dropping **only** the bulky `containers[].config` blob (the `services[].docker` roster is kept at every depth so the ladder is monotonic, `summary ⊆ status ⊆ full` — ADR-023 #1). **`describe_guest`** is the new single-VMID census (companion). The per-surface default split is deliberate: §1 stays opt-in (truncation loses data), §2 defaults-on (pseudo mounts are 0.0%-used noise, verdict preserved) — both are provably non-breaking for existing callers/tests.
...
- **Tier-aware census/health (§6):** below companion `describe_homelab`/`health_check` route metadata sections through `NodeOps` (API-complete: node/storage/containers/vms; health node/storage). Sections needing exec or a privilege the tier token lacks report a structured `{ unavailableAtTier: "companion" }` — census `network`/`services`/`tailscale`, health `units`/`guests`/`updates` (`updates` needs `Sys.Modify` the tier roles lack, so the apt/update API 403s — companion reads it via SSH; network + onboot are deferred-to-API follow-ups, documented in ADR §6). The drift differ treats `unavailableAtTier` as **not observed** (suppresses the sub-diff), never "removed".
...
**Three-layer topology — node → LXC → Docker — and the daemon socket is never exposed.** All `docker_*` tools (`docker_ps`/`docker_exec`/`docker_logs`/`docker_read_file`/`docker_write_file` plus the ADR-016 introspection trio `docker_inspect`/`docker_stats`/`compose_discover`) ride the **companion-tier `pct exec` plumbing** — they shell `docker …` *inside* a named LXC, so they inherit the SSH host-key/timeout/denylist boundary and never speak to `/var/run/docker.sock` directly. `TOOL_MIN_TIER` puts them all at **companion**; the backup-target kind `docker` resolves via `targetMinTier("docker") = "companion"`. Container names are charset-validated (`[a-zA-Z0-9][a-zA-Z0-9_.-]*`) before interpolation (`dockerHelpers.ts`, pure builders + parsers).

- **Introspection trio (ADR-016, read-only, not audited — siblings of `docker_ps`).** `docker_inspect(vmid, container, fields?)` runs `docker inspect <container>`, parses the JSON, and returns a structured secret-aware projection (`image` + `imageId` content-hash pin — container inspect exposes no registry `RepoDigest` without a 2nd round trip — `status`/`health`, `restartPolicy`, networks, mounts, ports, compose labels, and an env block that **keeps names but redacts values** via `redactRecord` on the *parsed* map, with `envRedactedCount`); `fields?` narrows the projection. `docker_stats(vmid)` is a `--no-stream` sample sorted by memory used desc (`parseDockerSize` handles binary + decimal suffixes). `compose_discover(vmid)` maps compose projects from running containers' `com.docker.compose.*` labels — it **produces the `composePath`** that `compose_redeploy`/`compose_preflight` otherwise require out-of-band (honest limit surfaced as a `note`: it sees running containers only). All three close the dogfooding gap where the introspection question had no tool and fell to hand-rolled `pct_exec docker inspect/stats` loops.

- **`docker_exec` is the 4th exec path** (after host `execute`, `pct_exec`, `qm_exec`): `pct exec <vmid> -- docker exec <container> sh -c '<cmd>'`. The inner command passes the **same two-tier denylist** (`confirm?: boolean` gates CONFIRM-tier). `docker_ps` parses tab-delimited `docker ps` rows (also reused by the census `services[].docker` slot, ADR-002). `docker_logs` is bounded and **always passes mandatory redaction** (mirrors `tail_log`); by deliberate contrast `docker_read_file` returns **full fidelity by default** (file bytes, like `read_file`) — with an **opt-in `redact` flag** (ADR-019) for the structure-over-secrets read.
- **File I/O is two-path (`dockerFiles.ts`).** A **host-visible fast path** resolves the container's host-visible source via `docker inspect` and degrades to the LXC `pct pull`/`pct push` flow; the **`docker cp` slow path** is a three-filesystem relay (Docker→LXC→Windows) for non-host-visible paths. ADR-016 §4 **broadened** the resolver (`isHostVisibleMount`): it now matches binds **plus local-driver named volumes** whose `Source` is under `/var/lib/docker/volumes/<name>/_data` (the linuxserver `/config` case), so managed-app config reads take the fast path instead of the relay; a non-local/NFS volume or tmpfs still falls back. The result field stays named `viaBindMount` (now "served by the host-visible fast path") to avoid churning the response/audit shape. **File content NEVER travels via argv** — only SFTP / `docker cp`. `docker_write_file` runs the full ADR-003 backup + audit pipeline and `dryRun` preview; backups key on a `docker:<vmid>:<container>:<path>` descriptor (no host/`pct`/`qm` collision), and `revert_file` routes `kind === "docker"` back through the bind-mount write flow. Docker targets are **excluded from the ADR-006 git mirror** (no descriptor-stable host filesystem) — `isHistoryTarget` returns false, like `qm`. `list_backups`/`diff_config` accept a `container` param (requires `vmid`) to address a docker target.
...
- **Seeded-vs-compared honesty (ADR-018 §1).** A seeding run reports an empty `drift` because **no detection occurred**, which is visually indistinguishable from a genuinely clean comparison. `VerifyReport` therefore carries an explicit **`mode: "seeded" | "compared"`** (the field consumers read), plus — when seeded — `seededReason` (`no-baseline` first-run / `level-changed` the silent re-seed a tracking-level or config change causes / `scope-new` reserved) and a human-readable `note`. `baselineSeeded` stays for back-compat. The classifier (`seededReasonFor`) and note (`seededNote`) are **pure + unit-tested**. **Both result-shape consumers learned it:** the ADR-015 drift-trend aggregator (`metrics/driftStats.ts`) derives its headline trend from **compared runs only** so a seed's `unexplained: 0` can't falsely flatten the curve (the series still marks the seed points), and the UI drift tab (`ui/page.ts`) renders a `status-warn` "Baseline RE-SEEDED — detection was NOT running" card for `level-changed`, so a re-seed never reads as "all clear." The batched L1 `find` enumeration that was ADR-018 §2 was already on master (`forest.ts`) — descoped at pressure-test.
...
## Derived operational metrics (ADR-015)

**"The artifacts already say what happened — derive the trend, don't add a sensor."** Three **pure aggregators** in `src/metrics/` that turn data the system already emits (the audit log, retained drift snapshots, the local backup store) into operator-facing summaries, rendered as a Metrics board in the ADR-010 UI. They are **renderer-only, not MCP tools** (no `TOOL_MIN_TIER` row) and there is **no `/metrics` scrape endpoint, ever** — that would be exactly the standing network-reachable surface ADR-010 §1 forbids. The honest-UI rule carries over: each panel is a `Panel<T>` with `available`/`snapshotTs`/`ageLabel`, never inventing liveness.

- **§1 audit statistics (`auditStats.ts`, `computeAuditStats`).** Over `AuditRecord[]` (windowed via the same `filterAuditRecords` core, ISO-prefix hour/day buckets): throughput series, by-tool + by-family counts (`toolFamily` → write/exec/read/other), and the **silent-failure signals** that matter — `historyMissRate` (history-eligible writes that didn't commit), `timedOutCount`, `signalKillCount` (`exitCode === null`, never coerced), `confirmGatedCount`, `rootTierCount`, and `unknownScopeRate` (exec-family `hashScope: "unknown"` — the ADR-009 drift-explanation gap). The renderer panel reads `audit.readAll()` **live** (anchored to the newest record's ts).
- **§2 drift-rate trend (`driftStats.ts`, `computeDriftTrend`).** Over the retained drift snapshots (`SnapshotStore.loadAll()`, oldest→newest by `savedAt`): per-run total/explained/unexplained/`l1OnlyTouches`/sensitive (`isSensitivePath` vs `integrity.sensitiveGlobs`)/seeded, plus headline `latestUnexplained`/`previousUnexplained`/`trend` (up/down/flat/insufficient-data off the last two runs)/`maxUnexplained`/`sensitiveEverNonZero`. The flagship panel — a rising unexplained count or any sensitive-path drift is the thing an operator most wants surfaced. **Honest deviation:** the persisted `VerifyReport` does not carry frozen/unavailable-prefix state, so counts derive from the drifted leaves only.
- **§3 backup-store health (`backupStats.ts`, `summarizeBackupStore`).** Over `BackupStore.storeStats()` (one `BackupStatEntry` per `.meta`): capacity (`totalBytes`/`headroomBytes`/`usedFraction`/`overCap` vs `globalSizeCapBytes`), version pressure (`targetsAtCap`/`targetsNearCap` vs `perFileVersionCap`), `kindMix` (`classifyKind` → delta / selfContained / metadataOnly), and ADR-014 `reanchorCount`/`reanchorFraction`. Computed **live** from the local store (`snapshotTs: null`, an explicit "Live" age label).
- **Config (`metrics.*` in `config.ts`):** `defaultWindowDays` (30), `defaultBucket` (`day`); env `METRICS_DEFAULT_WINDOW_DAYS`/`METRICS_DEFAULT_BUCKET`. The two thin I/O additions feeding the pure cores — `SnapshotStore.loadAll()` and `BackupStore.storeStats()` — stay **credential-free** so `ArtifactReader` can call them without breaking the `artifacts.test.ts` source-scan.
...
src/
  ...
  tools/
    ...
    readRedaction.ts    # Pure: applyReadRedaction — opt-in return-boundary redaction for the 4 read tools (ADR-019)
    ...
    serviceHelpers.ts   # Pure: systemctl show/restart builders + parseServiceShow (ADR-020 §1)
    serviceTools.ts     # service_status/_logs/_restart handlers (ADR-020 §1; service_logs delegates to tailLog)
    probes.ts           # tcp_ping/http_probe handlers + pure validators/curl-parser/ok-eval (ADR-020 §2)
    searchFileRegex.ts  # search_file_regex handler + pure buildGrepCommand/parseGrepContext (ADR-020 §3)
  ...
  guardrails/
    ...
    rollbackBreaker.ts  # Pure evaluateRollbackBreaker/rollbackTargetKey + stateful RollbackBreaker shell (ADR-021)
  backup/
  ...
  metrics/              # ADR-015 derived operational metrics (pure aggregators; renderer-only)
    auditStats.ts       # Pure: computeAuditStats — throughput/families/silent-failure signals over AuditRecord[]
    driftStats.ts       # Pure: computeDriftTrend — unexplained-leaves-per-run over retained drift snapshots
    backupStats.ts      # Pure: summarizeBackupStore — capacity/version-pressure/kind-mix/re-anchor frequency
```
(`ssh/hostKey.ts` is the SSH consumer of `trust/pinnedTrust.ts`. `src/ui/` is the only `src/` subtree that is NOT MCP-registered — it is the ADR-010 sidecar, run as a separate process. `src/metrics/` is pure aggregation consumed by the ADR-010 renderer (`ui/artifacts.ts`); it is not MCP-registered either.)

**Key invariant:** `guardrails/` (with one documented exception — `guardrails/rollbackBreaker.ts` is a pure core *plus* a thin stateful `RollbackBreaker` shell, the first guardrail to carry state; ADR-021), `backup/policy.ts`, `backup/eviction.ts`, `audit/record.ts`, `history/{paths,commitMessage,manifest,sweepPlanner}.ts`, `integrity/{folding,tree,forestShape,diff,classify,leafHash,acceptPolicy}.ts`, `tiers/{registry,rootFlag}.ts`, `ui/humanTools.ts` + the pure cores of `ui/{router,artifacts}.ts` (`isLoopbackAddress`/`routeUiRequest`, `snapshotAgeLabel`), `metrics/{auditStats,driftStats,backupStats}.ts` (ADR-015), and the decision core of `trust/pinnedTrust.ts` are **pure functions with no I/O** — the only way unit tests stay fast and trustworthy. (The git layer + the real-repo tests, the integrity `forest.ts`/`integrityEngine.ts` over a `MemoryNodeStore` + fake transport, and `ApiBackend` via an injected `ApiHttp` fixture, follow the same "pure core, thin I/O shell" split.)
...
- **Rollback circuit breaker — the one *cross-call* guardrail (ADR-021).** Every other guardrail judges a single call; the breaker (`guardrails/rollbackBreaker.ts`) catches a **thrash loop** across calls on the rollback family (`revert_file`/`snapshot_rollback`/`guest_backup_restore`). It refuses a verb once it has fired `limit` (default 3) times against the **same target** inside a sliding `windowMs` (default 10 min), keyed per target (`rollbackTargetKey`: the backup descriptor for file reverts, `guest/<vmid>` for the whole-guest verbs) so an honest revert of file A never trips a restore of guest 105. **Pure core** (`evaluateRollbackBreaker`/`rollbackTargetKey`/`breakerRefusal`) + a **thin stateful shell** (`RollbackBreaker`, a `Map<key, number[]>` — the first guardrail that *carries state*, `now` injected), one process singleton in `index.ts` injected into the three handlers. **Session = process lifetime** (a stdio server is one client over one long-lived process; ADR-021 resolved ADR-020's "what is a session" blocker with this), so a restart is the strongest reset and the window self-heals over time. On a trip the handler **audits the refusal** (`refused: true` + `circuitBreaker: {recentCount, limit, windowMs}` — the first audited *refusal*, since denylist/confirm refusals throw before append; it becomes an ADR-015 silent-failure signal) and throws a structured "hand back to a human / wait / re-issue with `overrideCircuitBreaker: true`" error. The override is **loud, not free**: it bypasses the breaker for that one call and stamps `circuitBreakerOverridden: true` (kept distinct from `confirm`), and an overridden call records no timestamp so the *next* non-override call re-trips the still-hot window. A tripwire, not a sandbox: a caller that always overrides, or varies its target each call, defeats it — the value is the forced pause and the audit trail. Config `guardrails.rollbackBreaker {enabled, limit, windowMs}` (env `ROLLBACK_BREAKER_ENABLED`/`_LIMIT`/`_WINDOW_MS`); `enabled: false` is a pure no-op kill switch.
...
- **Reads are fidelity-by-default, redaction-by-opt-in (ADR-019).** All four file-read tools (`read_file`/`pct_read_file`/`qm_read_file`/`docker_read_file`) take an optional `redact?: boolean`. **Absent ⇒ byte-for-byte unchanged** (the load-bearing default-invariance — no extra result fields, identical node-side read). With `redact: true` the returned text passes through the **same ADR-002 redaction module the log tools use** (`applyReadRedaction` at the *return boundary only*, `tools/readRedaction.ts`), adding `redacted`/`redactionCount`. It is **utf8-only** (a base64 read returns `redacted: false`, blob untouched) and **best-effort, not a security control**. Crucially it touches **only the returned payload** — `bytes` still reports the true pre-redaction length, and the backup/diff/integrity-hash/revert pipelines all keep using the true bytes (a write of a value previously read-with-redact must round-trip the real secret). This **amends ADR-004's "reads never redact" to "reads never redact *by default*."
