...
**Key invariant:** `guardrails/`, `backup/policy.ts`, `backup/eviction.ts`, `audit/record.ts`, `history/{paths,commitMessage,manifest,sweepPlanner}.ts`, `integrity/{folding,tree,forestShape,diff,classify,leafHash,acceptPolicy}.ts`, `tiers/{registry,rootFlag}.ts`, and the decision core of `trust/pinnedTrust.ts` are **pure functions with no I/O** — the only way unit tests stay fast and trustworthy. (The git layer + the real-repo tests, the integrity `forest.ts`/`integrityEngine.ts` over a `MemoryNodeStore` + fake transport, and `ApiBackend` via an injected `ApiHttp` fixture, follow the same "pure core, thin I/O shell" split.)
