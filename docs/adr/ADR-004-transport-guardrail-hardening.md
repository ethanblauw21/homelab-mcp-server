...
**Required by:** ADR-016 (read caps + path validation for the `docker_read_file` named-volume fast path), ADR-019 (amends the "reads never redact" doctrine to "fidelity by default, opt-in redaction"), ADR-020 (denylist/confirm gate, `validatePath`, honest `ExecResult` exit semantics for the service/probe/search tools), ADR-021 (the denylist/confirm **tripwire doctrine** + pure-guardrail-function pattern, extended cross-call by the rollback circuit breaker), ADR-022 (the JSONL audit log + `O_APPEND` durability that `audit.db` shadows but never replaces, the redaction module the stored diffs/change-events reuse, and the diff-on-write `computeUnifiedDiff` output — formerly discarded — now persisted per write-family record)
