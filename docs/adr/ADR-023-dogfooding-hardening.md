...
| **B5** | `snapshot_rollback` verifies the post-rollback restart by POLLING run-state (`pct/qm status`) until running or a bounded deadline, instead of trusting the start command's `exitCode` | **Shipped + unit-tested (2026-06-23, follow-up branch)** | New `waitForGuestRunning` poll (bounded by `snapshot.restartPollIntervalMs`/`restartTimeoutMs`, injectable sleep). Tests pin both the false-failure regression (start `exitCode:null` + running ⇒ success) and the genuine failure (stays non-running past the deadline ⇒ throw). Live rollback-cycle re-test still pending a maintenance window / disposable guest. |
