...
- `health_check` likewise: at observe the API path serves `node` (load/memory), `storage` (PVE stores), and `updates` (apt simulate via `aptUpdates()`); the failed-systemd-units check (`units`) is exec-bound ⇒ `unavailableAtTier`. **Implementation note:** onboot-vs-status detection needs `/etc/pve/*.conf` (the onboot flag is not in the API guest list), so the `guests` section is also `unavailableAtTier: "companion"` below companion until a config-backed API path is added. ZFS and per-filesystem `df` usage have no token-grade API and are simply omitted (not errors).
